...
function Form({taches,setTaches,tache,setTache}) {
  ...
  function Next(e) {
    ...
    if ( tache.id === "" && tache.nom === "" && tache.prenom === "" && tache.email === "" && tache.tele === "" && tache.nom_tache === ""){
      ...
      setTache({id:"", nom: "", prenom: "", email: "", tele: "", nom_tache: "" });
      Navigate("/Table");
  ...
  return (
    ...
    <section className="bg-white p-1 xs:p-8">
      <div className="max-w-96 sm:max-w-4xl mx-auto border border-[#4D7C0F] rounded-lg p-8">
        <h2 className="sm:text-xl text-[12px] font-bold mb-6 text-[#4D7C0F]">Information Personnelle</h2>
        <form>
          <div className="space-y-6">
            ...
            <button
              onClick={Next}
              type="submit"
              className="sm:w-[86px] w-full h-[50px] text-xs sm:text-base bg-[#4D7C0F] rounded-[5px] p-[13px_25px] text-white"
            >
              Next
            </button>
